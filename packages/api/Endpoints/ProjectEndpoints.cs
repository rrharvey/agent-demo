...
public class ProjectEndpoints : IEndpointMapper
{
  public void MapEndpoints(WebApplication app)
  {
    ...
    app.MapGet(
      ...
      {
        var project = await db.Projects.Include(p => p.Client).FirstOrDefaultAsync(p => p.Id == id);
    ...
    app.MapPost(
      "/api/projects",
      async (CreateProjectDto projectDto, ApplicationDbContext db) =>
      {
    ...
    app.MapPut(
      "/api/projects/{id}",
      async (int id, UpdateProjectDto projectDto, ApplicationDbContext db) =>
      {
    ...
    app.MapGet(
      ...
      async (int clientId, ApplicationDbContext db) =>
      {
        ...
        var projects = await db
          .Projects.Where(p => p.ClientId == clientId)
          .Select(p => new ProjectDto(
            p.Id,
            p.Name,
            p.Description,
