...
public class TimeEntryEndpoints : IEndpointMapper
{
  public void MapEndpoints(WebApplication app)
  {
    app.MapGet(
      "/api/time-entries",
      async ([FromQuery] string userId, ApplicationDbContext db) =>
      {
        ...
        return Results.Ok(
          await db
            .TimeEntries.Where(t => t.UserId == userId)
            .Include(t => t.Project)
    ...
    app.MapPost(
      "/api/time-entries",
      async (CreateTimeEntryDto entryDto, ApplicationDbContext db) =>
      {
    ...
    app.MapPut(
      "/api/time-entries/{id}",
      async (int id, UpdateTimeEntryDto entryDto, ApplicationDbContext db) =>
      {
    ...
    app.MapGet(
      ...
      async (int projectId, ApplicationDbContext db) =>
      {
        var project = await db
          .Projects.Include(p => p.Client)
        ...
        var entries = await db
          .TimeEntries.Where(t => t.ProjectId == projectId)
          .Select(t => new TimeEntryDto(
            t.Id,
            t.Description,
            t.StartTime,
    ...
    app.MapPut(
      ...
      async (int id, ApplicationDbContext db) =>
      {
        ...
        if (entry.EndTime != null)
          return Results.BadRequest("Time entry is already stopped");
        ...
        return Results.Ok(
          new TimeEntryDto(
            ...
            entry.Project != null && entry.Project.Client != null
