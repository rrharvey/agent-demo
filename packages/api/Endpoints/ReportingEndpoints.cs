...
public class ReportingEndpoints : IEndpointMapper
{
    public void MapEndpoints(WebApplication app)
    {
        ...
        app.MapGet("/api/reports/time-entries", GetTimeEntriesReport);

        // Get summary by project
        app.MapGet("/api/reports/summary/by-project", GetProjectSummary);

        // Get summary by client
        app.MapGet("/api/reports/summary/by-client", GetClientSummary);

        // Get summary by user
        app.MapGet("/api/reports/summary/by-user", GetUserSummary);
    }

    private static async Task<IResult> GetTimeEntriesReport(
        ...
        DateTime? endDate)
    {
        ...
        var summaries = await query
            ...
            .Select(g => new 
            {
                ...
                TotalHours = g.Sum(t => t.EndTime.HasValue 
                    ? (t.EndTime.Value - t.StartTime).TotalHours 
                    : 0),
                BillableHours = g.Where(t => t.IsBillable).Sum(t => t.EndTime.HasValue 
                    ? (t.EndTime.Value - t.StartTime).TotalHours 
                    : 0),
                NonBillableHours = g.Where(t => !t.IsBillable).Sum(t => t.EndTime.HasValue 
                    ? (t.EndTime.Value - t.StartTime).TotalHours 
                    : 0)
            })
            .ToListAsync();
    ...
    private static async Task<IResult> GetUserSummary(
        ApplicationDbContext db,
        DateTime? startDate,
        DateTime? endDate)
    {
