...
public class ClientEndpoints : IEndpointMapper
{
    public void MapEndpoints(WebApplication app)
    {
        ...
        app.MapGet("/api/clients/{id}", async (int id, ApplicationDbContext db) =>
            await db.Clients.FindAsync(id) is Client client
                ? Results.Ok(new ClientDto(client.Id, client.Name, client.Description, client.IsActive, client.CreatedAt))
                : Results.NotFound());

        app.MapPost("/api/clients", async (CreateClientDto clientDto, ApplicationDbContext db) =>
        {
        ...
        app.MapPut("/api/clients/{id}", async (int id, UpdateClientDto clientDto, ApplicationDbContext db) =>
        {
